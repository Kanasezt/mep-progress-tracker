...
if not df_raw.empty:
    # --- FIXED: Handle GMT+7 Conversion safely ---
...
if page == "upload":
    ...
    if not df_raw.empty:
        ...
        if not df_f.empty:
            # --- FIXED: ADD CSV EXPORT HERE ---
            csv = df_f.to_csv(index=False).encode('utf-8-sig')
            st.download_button("📥 Download Progress (CSV)", data=csv, file_name="mep_progress.csv", mime="text/csv")
