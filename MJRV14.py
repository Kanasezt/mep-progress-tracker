...
st.set_page_config(page_title="MEP Tracker V25 Admin", layout="wide")

# --- ส่วนแก้ไขสำหรับ iPhone (Responsive Fix) ---
st.markdown("""
    <style>
    /* ลด Padding รอบตัวแอปให้พอดีจอ iPhone */
    .block-container {
    ...
    /* แก้ไขปัญหาตารางหรือกราฟดันขอบจนล้น */
    .stPlotlyChart {
        width: 100% !important;
    }
    
    /* ปรับปุ่ม Submit ให้เต็มหน้าจอในมือถือเพื่อให้กดง่ายขึ้น */
    ...
    /* บังคับให้ช่องกรอกข้อมูลไม่กว้างเกินหน้าจอ */
...
# --- 5. Dashboard ---
if is_upload_only:
    # สำหรับหน้าจอ iPhone ลูกน้อง จะแสดงเฉพาะฟอร์ม (ไม่มี Sidebar ล้นออกมา)
    show_upload_form()
else:
    with st.sidebar:
        show_upload_form()
    ...
    if not df_raw.empty:
        ...
        if not df_filtered.empty:
            ...
            # ใช้ช่องว่างคงที่เพื่อให้คอลัมน์ตรงกัน
