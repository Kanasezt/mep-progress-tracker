...
# --- 7. Admin Panel (ปรับปรุงใหม่ให้กดง่ายขึ้น) ---
with st.sidebar:
    ...
    if pwd == "pm1234":
        ...
        if not df.empty:
            st.write("---")
            # 1. เลือกรายการที่ต้องการแก้ (แสดง ID และชื่อคนแจ้ง)
            options = {row['id']: f"ID:{row['id']} - {row['staff_name']}" for _, row in df.iterrows()}
            target_id = st.selectbox("1. เลือก ID ที่จะแก้ไข", 
            ...
            # 3. ปุ่มกดยืนยัน (ต้องกดปุ่มนี้เท่านั้นถึงจะเปลี่ยน)
            if st.button("🚀 บันทึกการเปลี่ยนแปลง", type="primary"):
                try:
                    # ส่งคำสั่งไปที่ Supabase
                    supabase.table("issue_escalation").update({"status": new_status}).eq("id", target_id).execute()
                    
                    # ล้างแคชเพื่อให้หน้าจอแสดงข้อมูลล่าสุดทันที
                    st.cache_data.clear() 
                    st.success(f"อัปเดต ID {target_id} เป็น {new_status} เรียบร้อย!")
