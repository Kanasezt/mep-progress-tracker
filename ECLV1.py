...
st.set_page_config(page_title="Issue Escalation V2.5", layout="wide")

# --- 2. CSS Styling (สีส้มเข้ม / เขียวเข้ม / เทาเข้ม) ---
st.markdown("""
    <style>
...
# --- 4. Overall Status ---
st.title("🚨 Issue Escalation Portal V2.5")
...
if not df.empty:
    ...
    for i, r in df_f.reset_index(drop=True).iterrows():
        ...
        # --- Fix Date Error (Check if Null) ---
        if pd.notnull(r['created_at']):
            # ปรับ timezone ให้คำนวณวันได้
...
with st.sidebar:
    ...
    if pwd == "pm1234":
        ...
        if not df.empty:
            # แสดงชื่อคนแจ้งประกอบการเลือก ID
